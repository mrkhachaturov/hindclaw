...
The OpenAPI spec itself lives at `apps/docs/static/openapi.json` (served by
the docs site and consumed by every client generator). It is no longer duplicated
under `packages/clients/`.
...
- `extract-openapi.py` builds a FastAPI app from HindclawHttp and writes the
  spec directly to `apps/docs/static/openapi.json` (no running server
...
Prerequisites: Docker, Go 1.18+, Rust toolchain (cargo), Node.js + npm, `pip install -e packages/extension/` + `hindsight-api-slim`

## When to Regenerate

Regenerate after any change to:
- Route decorators in `packages/extension/hindclaw_ext/http.py` (new endpoints, changed paths)
- Response/request models in `packages/extension/hindclaw_ext/http_models.py` (new fields, new models)
...
- **Go module path**: `github.com/mrkhachaturov/hindclaw/packages/clients/go`
